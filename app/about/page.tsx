...
export default function AboutPage() {
  return (
    <>
      ...
      {/* A full-screen badge pinned to the viewport, which everything below
          then scrolls up over while it fades out. */}
      <ScrollAwayLogo />

      {/* `relative z-10` is not decoration. The badge's layer is `fixed`, so it
          is positioned, and a positioned element paints above static ones
          regardless of document order — without a stacking context of its own
          this content would scroll *under* the backdrop it is meant to cover.
          One wrapper rather than a z-index on each Sheet, so a section added
          later cannot forget it. */}
      <div className="relative z-10">
        {/* This first panel alone drops `bg-sheet` for `bg-transparent`: it is
            the one that overlaps the badge, and an opaque panel would simply
            hide what it is supposed to be reading over. Safe to do because
            ground and sheet sit a hairline apart in luminance on cream — once
            the badge has gone, this section is indistinguishable from the
            opaque ones below it. */}
        <Sheet className="bg-transparent" innerClassName="py-10 sm:py-14 lg:py-16">
