...
Yarns and Buttons has **no single primary user** — this was confirmed, not defaulted. Three real audiences share the storefront and design decisions are made per surface rather than from one ranked persona:
...
- **The reader does not always arrive at the top of the page.** Browsers restore scroll position on reload and on back navigation, and that restore lands *after* hydration. Anything seeded at mount from "where the page is" therefore reads 0 and stays there until the next scroll — both `useScroll` forms do exactly this, which left `/about`'s fixed full-screen logo painting at full opacity over the footer. Scroll-linked work reads `window.scrollY` directly and re-seeds on the following frames (`ScrollAwayLogo.tsx`). This is the cart-store bug in different clothing: state that assumes it begins at the beginning. Test it by scrolling to the bottom and reloading.
- **Animation is transform/opacity only and `prefers-reduced-motion` aware.** Framer Motion is already in the stack. Reduced motion must not merely slow an effect down — a scroll-linked fade ties whether content is *visible* to movement, so its reduced-motion branch renders the content plainly and animates nothing.
...
- **No component-level test coverage.** Nothing in the project mounts React — no Playwright, no React Testing Library. A type-check, lint, 324 unit tests and a clean build have all passed over a component that rendered the wrong branch. Visual changes are verified by loading pages, or not at all — and when they are, assert on the property under test rather than on document position: a check selecting an element as `img[1].parentElement` twice reported a bug still present while the page was visibly correct.
